...
if (strlen($_SESSION['sturecmsaid']) == 0) {
    ...
    if (isset($_POST['submit12'])) {
        ...
        $num = $_POST['num'];
        $date = date('Y-m-d'); 
        $selected_class = $_POST['classID']; 

        for ($i = 1; $i <= $num; $i++) {
            $name_attendance = "ATTENDANCE" . $i;
            $attendance = $_POST[$name_attendance];
            $nameID = "ID" . $i;
            $studentID = $_POST[$nameID];

            // Debugging output
            //echo "date: " . $date . " studentID: " . $studentID . " classID: " . $selected_class . " courseID: " . $CourseID . "<br>";

            if (isset($attendance)) {
                // Check if the attendance record already exists for the student on the given date
                $sql_check = "SELECT * FROM tblattendance WHERE StudentID = :studentID AND ClassID = :classID AND AttendanceDate = :date";
                $query_check = $dbh->prepare($sql_check);
                $query_check->bindParam(':studentID', $studentID, PDO::PARAM_INT);
                $query_check->bindParam(':classID', $selected_class, PDO::PARAM_INT);
                ...
                if ($result) {
                    ...
                    $sql_update = "UPDATE tblattendance SET IsPresent = :attendance WHERE StudentID = :studentID AND ClassID = :classID AND AttendanceDate = :date";
                    $query_update = $dbh->prepare($sql_update);
                    $query_update->bindParam(':attendance', $attendance, PDO::PARAM_INT);
                    $query_update->bindParam(':studentID', $studentID, PDO::PARAM_INT);
                    $query_update->bindParam(':classID', $selected_class, PDO::PARAM_INT);
                    $query_update->bindParam(':date', $date, PDO::PARAM_STR);
                    $query_update->execute();
                    
                } else {
                    // Insert a new attendance record
                    $sql_insert = "INSERT INTO tblattendance (StudentID, ClassID, AttendanceDate, IsPresent) VALUES (:studentID, :classID, :date, :attendance)";
                    $query_insert = $dbh->prepare($sql_insert);
                    $query_insert->bindParam(':studentID', $studentID, PDO::PARAM_INT);
                    $query_insert->bindParam(':classID', $selected_class, PDO::PARAM_INT);
                    $query_insert->bindParam(':date', $date, PDO::PARAM_STR);
                    $query_insert->bindParam(':attendance', $attendance, PDO::PARAM_INT);
                    $query_insert->execute();
                    
                }
            }
        }
        //header('location:view-attendances.php');
...
<head>
    <titleStudent  Information System ||| Manage Students</title>
    <!-- plugins:css -->
                                ...
                                <div class="card-body">
                                    <form action="" method="post">
                                        <div class="d-sm-flex align-items-center mb-4">
                                            <h4 class="card-title mb-sm-0">Manage Students Attendance 
                                            <?php 
                                        ...
                                        <div class="table-responsive border rounded p-1">
                                            <table class="table">
                                                ...
                                                <tbody>
                                                    ...
                                                    // Assuming $selected_class, $selected_course, and $date are already set
                                                    if ($selected_class != '' && $selected_course != '') {
                                                        echo "<input type='hidden' name='CourseID' value='" . $selected_course . "'>";
                                                        echo "<input type='hidden' name='classID' value='" . $selected_class . "'>";
                                                        // Fetch students enrolled in the class
                                                        $sql_students = "SELECT *, tblstudent.ID as studentID FROM tblstudent WHERE StudentClass = :class GROUP BY tblstudent.ID";
                                                        ...
                                                        foreach ($students as $key => $student) {
                                                            ...
                                                            if ($attendance_result) {
                                                                ...
                                                                // Display a dropdown to select attendance status if no record exists
                                                                $name_attendance = "ATTENDANCE" . $i;
                                                                echo "<td><select name='$name_attendance' class='form-control' required>";
                                                                echo "<option selected disable>Select option</option>";
                                                                echo "<option value='1'>Present</option>";
                                                                echo "<option value='0'>Absent</option>";
                                                                echo "</select></td>";
                                                                $id_name = "ID" . $i;
                                                                echo "<td><input type='hidden' name='$id_name' value='" . $student['studentID'] . "' required></td>";
                                                                echo "</tr>";
                                                            }
                                                        }
                                        ...
                                        <center><button type="submit" name="submit12" class="btn btn-primary">Save Grades and Attendance</button></center>
                                    </form>
                                </div>
                            </div>
                        </div>
